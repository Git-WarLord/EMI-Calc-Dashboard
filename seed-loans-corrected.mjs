...
// Calculate closing month for each loan
// Starting from July 2026, if remainingEMIs = 1, closes in Jul 2026
// If remainingEMIs = 2, closes in Aug 2026, etc.
function calculateClosingMonth(remainingEMIs) {
  const startDate = new Date(2026, 6, 1); // July 2026
  const endDate = new Date(startDate);
  // Add (remainingEMIs - 1) months to get the closing month
  endDate.setMonth(endDate.getMonth() + remainingEMIs - 1);
...
async function seedLoans() {
  ...
  try {
    ...
    for (const loan of loansData) {
      ...
      console.log(`✓ ${loan.name.padEnd(20)} | ₹${loan.monthlyEMI.toString().padStart(6)} x ${loan.remainingEMIs.toString().padStart(2)} EMIs | Closes: ${closingMonth.padEnd(10)} | Total: ₹${totalRemaining.toString().padStart(8)}`);
    }
    
    // Verify the data
    const [allLoans] = await connection.query('SELECT * FROM loans WHERE userId = ? ORDER BY dueDate', [userId]);
    console.log(`\n${'='.repeat(100)}`);
    console.log(`✓ Total loans seeded: ${allLoans.length}`);
    ...
    console.log(`✓ Timeline: July 2026 → ${allLoans[allLoans.length - 1].closesIn}`);
    console.log(`${'='.repeat(100)}\n`);
    
    console.log('Detailed Loan Schedule:');
    allLoans.forEach(loan => {
      console.log(`  ${loan.name.padEnd(20)} | Due: ${loan.dueDate.padEnd(4)} | ₹${parseFloat(loan.monthlyEMI).toString().padStart(6)} x ${loan.remainingEMIs.toString().padStart(2)} = ₹${parseFloat(loan.totalRemaining).toString().padStart(8)} | Closes: ${loan.closesIn}`);
    });
